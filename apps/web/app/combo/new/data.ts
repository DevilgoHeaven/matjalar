...
export interface ComboFormCatalog {
  brand: {
    id: string;
    name: string;
    slug: string;
...
export async function getComboFormCatalog(): Promise<ComboFormCatalog | null> {
  ...
  return {
    ...
    variants: (variants ?? [])
      ...
      .filter((variant): variant is ComboFormCatalog['variants'][number] => variant !== null)
      .sort((a, b) => {
