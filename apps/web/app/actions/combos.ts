...
export type CheckSimilarCombosResult =
  | { ok: true; combos: SimilarCombo[] }
...
const registerComboSchema = z.object({
...
/**
 * 조합 등록 전 같은 combo_signature 를 가진 공개/대기 조합을 찾는다.
 */
export async function checkSimilarCombos(
  formData: FormData
): Promise<CheckSimilarCombosResult> {
  try {
    const parsed = registerComboSchema.safeParse({
    ...
    if (selections.length === 0) {
      return { ok: false, error: '최소 1개 이상의 옵션을 선택해 주세요.' };
    }

    const { variant, brand, normalizedOptions } = await prepareComboDraft({
      menuVariantId: parsed.data.menuVariantId,
    ...
    const comboSignature = await buildComboSignature({
      ...
      options: buildSignatureOptions(normalizedOptions),
    });

    return { ok: true, combos: await loadSimilarCombos(comboSignature) };
  } catch (error) {
    return {
      ok: false,
      error: error instanceof Error ? error.message : '유사 조합 확인에 실패했습니다.',
    };
  }
}

/**
 * 회원이 제출한 조합을 pending 상태로 저장한다.
 */
export async function registerCombo(formData: FormData): Promise<void> {
  const parsed = registerComboSchema.safeParse({
    title: formData.get('title'),
    menuVariantId: formData.get('menuVariantId'),
    tagIds: getStringValues(formData, 'tagIds'),
  ...
  if (!(await isCurrentAppUserActive())) {
    throw new Error('현재 계정 상태에서는 조합을 등록할 수 없습니다.');
  }

  const { variant, menu, brand, selectedTags, normalizedOptions } =
    await prepareComboDraft({
      menuVariantId: parsed.data.menuVariantId,
      tagIds: parsed.data.tagIds,
      selections,
    });
  ...
  const similarCombos = await loadSimilarCombos(comboSignature);
  if (!allowDuplicate && similarCombos.length > 0) {
    throw new Error('이미 비슷한 조합이 있습니다. 확인 후 계속 등록해 주세요.');
  }
  ...
  try {
    ...
    if (cleanupError) {
      console.warn('[registerCombo] pending combo cleanup failed:', cleanupError.message);
    }
    throw error;
  }

  revalidatePath('/admin/combos');
  redirect(`/combo/new?submitted=1&comboId=${comboId}`);
}

async function prepareComboDraft(input: {
  ...
  const [groups, items, selectedTags] = await Promise.all([
    ...
    loadSelectedOptionItems(input.selections.map((selection) => selection.optionItemId)),
    loadSelectedTags(input.tagIds),
  ]);
  ...
  return { variant, menu, brand, selectedTags, normalizedOptions };
}

async function loadSimilarCombos(comboSignature: string): Promise<SimilarCombo[]> {
  const db = asSupabaseQueryClient(await getSupabaseServerClient());
  const { data, error } = await db.rpc<SimilarCombo[]>('find_similar_combos', {
    p_signature: comboSignature,
    p_limit: 3,
  });
