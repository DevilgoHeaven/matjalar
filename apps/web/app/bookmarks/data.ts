...
type BookmarkRow = Pick<TableRow<'bookmarks'>, 'combo_id' | 'created_at'>;
type ComboRow = Pick<
  TableRow<'combos'>,
  | 'id'
  | 'title'
...
export async function getBookmarkedCombos(
  userId: string
): Promise<BrandListCombo[]> {
  const supabase = await getSupabaseServerClient();
  const db = asSupabaseQueryClient(supabase);
  const { data: bookmarks, error: bookmarksError } = await db
    .from('bookmarks')
    .select<BookmarkRow>('combo_id, created_at')
    .eq('user_id', userId)
  ...
  const comboIds = bookmarkRows.map((bookmark) => bookmark.combo_id);
  ...
  const [statsByComboId, tagsByComboId] = await Promise.all([
    getStatsByComboId(comboRows.map((combo) => combo.id)),
    getTagsByComboId(comboRows.map((combo) => combo.id)),
  ]);
  const comboById = new Map(comboRows.map((combo) => [combo.id, combo]));

  return bookmarkRows
    .map((bookmark) => comboById.get(bookmark.combo_id))
    .filter((combo): combo is ComboRow => Boolean(combo))
    .map((combo) => {
